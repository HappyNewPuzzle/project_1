...
플레이어의 학습용 월드 위치는 `/pos`로 확인하고 `/move`로 변경합니다.
...
같은 방 안에서 시야 거리 안에 있는 플레이어는 `/nearby`로 확인합니다.
...
현재 위치에 플레이어가 나타났다는 사실은 `/spawn`으로 주변 플레이어에게 알릴 수 있습니다.
...
```text
> /help
< [notice] Commands: /help, /commands, /name <nickname>, /rename <nickname>, /whoami, /session, /login <playerId>, /pos, /move <x> <y>, /nearby, /spawn, /despawn, /users, /rooms, /room-users, /stats, /motd, /version, /join <room>, /leave, /where, /ping, /echo <message>, /time, /uptime, /me <action>, /whisper <nickname> <message>, /quit
```
